...
function Graph(method, url, postData, callback) {
  ...
  if (postData.appSecret) {
    this.appSecret = postData.appSecret;
    delete postData.appSecret;
  }
...
Graph.prototype.cleanUrl = function(url) {
  ...
  var sessionAccessToken = accessToken;
  if (url.indexOf('access_token=') !== -1) {
    var regex = /access_token=([^&]*)/;
    var results = regex.exec(url);

    if (results)
      sessionAccessToken = results[1];
  } else if (accessToken) {
    url += ~url.indexOf('?') ? '&' : '?';
  ...
  if (sessionAccessToken && this.appSecret && url.indexOf('appsecret_proof') === -1) {
...
exports.get = function(url, params, callback) {
  ...
  if (params)  {
    if (params.appSecret) {
      postData.appSecret = params.appSecret;
  ...
  return new Graph('GET', url, postData, callback);
};
